...
use crate::sftp_parse::{
    FsEntry, local_root, parse_ls, parse_pwd,
    read_local_dir, scrape_transfer_progress, shell_quote, strip_ansi,
};
...
pub enum BrowserFocus { Local, Remote }
...
impl FileBrowser {
        ...
        match self.sftp_state {
            ...
            SftpState::WaitingPwd => {
                if prompt_ready {
                    ...
                    self.remote_path = parse_pwd(&lines).unwrap_or_else(|| self.remote_path.clone());
            ...
            SftpState::Transferring => {
                if prompt_ready {
                    self.prompt_stable = 0;
                    if let Some(ref mut t) = self.last_transfer { t.done = true; t.progress = "100%".to_string(); }
                    ...
                    if let Some(pct) = scrape_transfer_progress(&lines) {
                        if let Some(ref mut t) = self.last_transfer { t.progress = pct; self.needs_redraw = true; }
                    }
                }
            }
        ...
        match self.focus {
            BrowserFocus::Local => {
                if let Some(i) = self.local_sel.selected() {
                    ...
                    if entry.name == ".." {
                        if let Some(p) = self.local_path.parent() { self.local_path = p.to_path_buf(); }
                        else { self.local_path = PathBuf::from(local_root()); }
    ...
    pub fn download(&mut self) {
        ...
        if let Some(i) = self.remote_sel.selected() {
            ...
            self.last_transfer = Some(TransferStatus { filename: entry.name.clone(), done: false, progress: "0%".to_string() });
    ...
    pub fn delete_focused(&mut self) {
        match self.focus {
            ...
            BrowserFocus::Remote => {
                if let Some(i) = self.remote_sel.selected() {
                    let entry = if let Some(e) = self.remote_entries.get(i).cloned() { e } else { return; };
                    if entry.name == ".." || self.sftp_state != SftpState::Idle { return; }
    ...
    pub fn drag_local_to_remote(&mut self) { self.upload(); }
    pub fn drag_remote_to_local(&mut self) { self.download(); }
    ...
    pub fn render(&mut self, area: Rect, buf: &mut Buffer, is_focus: bool, leaf_count: usize) {
        let inner = if leaf_count > 1 {
            ...
            let block = Block::default().borders(Borders::ALL).border_style(border_style).title(title.as_str());
        ...
        let status_area = Rect { y: inner.y + inner.height.saturating_sub(status_h), height: status_h, ..inner };
    ...
    fn render_panel(&mut self, area: Rect, buf: &mut Buffer, side: BrowserFocus, pane_focused: bool) {
        ...
        let (title, path_str, entries, list_state) = match side {
            BrowserFocus::Local  => (" local ",  self.local_path.to_string_lossy().to_string(), &self.local_entries,  &mut self.local_sel),
            BrowserFocus::Remote => (" remote ", self.remote_path.clone(),                       &self.remote_entries, &mut self.remote_sel),
        };
        ...
        let block = Block::default()
            ...
            .title_bottom(Span::styled(format!(" {} ", path_str), Style::default().fg(Color::DarkGray)));
        let inner = block.inner(area);
        block.render(area, buf);
        ...
        if side == BrowserFocus::Remote && only_dotdot && !self.raw_snapshot.is_empty() {
            let visible: Vec<&String> = self.raw_snapshot.iter().filter(|l| !l.trim().is_empty()).collect();
            let start = visible.len().saturating_sub(inner.height as usize);
            for (i, line) in visible[start..].iter().enumerate() {
                let y = inner.y + i as u16;
                if y >= inner.y + inner.height { break; }
                let span = Span::styled(line.chars().take(inner.width as usize).collect::<String>(), Style::default().fg(Color::DarkGray));
        ...
        let items: Vec<ListItem> = entries.iter().map(|e| {
            ...
            let line = Line::from(vec![
                ...
                Span::styled(format!(" {:>W_SIZE$}", e.size),     Style::default().fg(Color::DarkGray)),
                Span::styled(format!(" {:<W_MOD$}", e.modified),  Style::default().fg(Color::DarkGray)),
                Span::styled(format!(" {:<W_PERMS$}", e.perms),   Style::default().fg(Color::DarkGray)),
            ]);
            ListItem::new(line)
        }).collect();

        let list = List::new(items)
            .highlight_style(Style::default().fg(Color::Black).bg(if is_active { Color::Cyan } else { Color::DarkGray }).add_modifier(Modifier::BOLD))
            .highlight_symbol("> ");
    ...
    fn render_status(&self, area: Rect, buf: &mut Buffer) {
        ...
        let transfer_str = if let Some(ref t) = self.last_transfer {
            if t.done { format!("✓ {}  ", t.filename) } else { format!("⟳ {} {}  ", t.filename, t.progress) }
        } else { String::new() };

        let parse_hint = if self.remote_entries.len() <= 1 && !self.raw_snapshot.is_empty() {
            self.raw_snapshot.iter().rev().find(|l| !l.trim().is_empty())
        ...
        buf.set_line(area.x, area.y, &Line::from(vec![
            ...
            Span::styled(format!(" {}{}  {}  {}", self.status_msg, parse_hint, transfer_str, help), Style::default().fg(Color::DarkGray)),
        ]), area.width);
    }
}
