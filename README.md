...
- Scrollback in interactive sessions, with mouse scroll support
...
Scrollback: mouse scroll navigates the output history when the remote app is not capturing mouse. In alternate screen apps (vim, htop, less), scroll sends arrow keys instead. Any keypress snaps back to live view.
...
| `input/` | All key and mouse dispatch (global shortcuts, connect, session, browser, mouse) |
| `app.rs` | `App` state: tabs, host list, session/browser creation |
| `pane.rs` | `Pane` leaf enum + pane-specific operations over the split tree |
| `components/pane_tree/` | Generic `SplitTree<T>` tiling layout (`tree.rs`) + `PaneTreeView` renderer (`view.rs`) |
| `components/browser/` | `BrowserCore` shared state machine (`state.rs` + behaviour modules), SFTP and SCP drivers (`sftp.rs`, `ssh.rs`), `ls -la` parsing (`parse.rs`), dual-panel rendering (`view.rs`) |
| `components/terminal/` | `EmbeddedTerminal` — wraps `crate::pty` + vt100, exposes `PtyChannel` trait, `MockPty` for tests (`session.rs`); screen-grid rendering (`view.rs`) |
| `components/connect/` | Connect pane state — host list, overlays, key editor (`state.rs`) — and its rendering (`view.rs`) |
| `components/bottom_bar.rs` | Tab strip |
| `components/overlays.rs` | Exit overlay + right-click context menu |
...
"Stable" means the raw PTY buffer has not changed for a few consecutive ticks (`PROMPT_STABLE_TICKS`) and the last non-empty line contains `sftp>`. This prevents acting on a prompt that appears mid-output before all data has been flushed.
...
```bash
cargo test --lib    # unit tests (inline in source files)
```
